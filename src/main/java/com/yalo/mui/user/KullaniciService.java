...
public class KullaniciService {
    ...
    @Transactional(readOnly = true)
    public Kullanici findByGiris(KullaniciGiris kullaniciGiris) throws UserNotFoundException {

        Kullanici kullanici = kullaniciRepo.findKullaniciByEmailAndPassword(kullaniciGiris.getEmail(), kullaniciGiris.getPassword());
        if (kullanici == null) {
            throw new UserNotFoundException(" Girilen bilgilerle uyumlu kullanıcı Bulunamadı. ");
        }
        return kullanici;
    }
